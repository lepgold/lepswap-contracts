...
describe("MasterLep", function () {
  ...
  it("should set correct state variables", async function () {
    ...
    const sushi = await this.chef.lep()
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should give proper SUSHIs allocation to each pool", async function () {
      ...
      expect(await this.chef.pendingLep(0, this.alice.address)).to.equal("10000")
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo("424")
      await this.chef.connect(this.bob).deposit(1, "5", { from: this.bob.address })
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await this.chef.pendingLep(0, this.alice.address)).to.equal("11666")
      await advanceBlockTo("430")
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chef.pendingLep(0, this.alice.address)).to.equal("13333")
      expect(await this.chef.pendingLep(1, this.bob.address)).to.equal("3333")
    })
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async function () {
      ...
      expect(await this.chef.pendingLep(0, this.alice.address)).to.equal("10500")
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      expect(await this.chef.pendingLep(0, this.alice.address)).to.equal("0")
